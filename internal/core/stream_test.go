...
func TestStream_IsDirectionIn(t *testing.T) {
	t.Run("test bit set", func(t *testing.T) {
		assert := base.NewAssert(t)
		for i := 0; i < 256; i++ {
			v := NewStream()
...
func TestStream_SetDirectionOut(t *testing.T) {
	t.Run("test", func(t *testing.T) {
		assert := base.NewAssert(t)
		for i := 0; i < 256; i++ {
			v := NewStream()
		...
		for _, testData := range streamTestSuccessCollections["bool"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					...
					assert(stream.ReadBool()).Equal(false, errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
		...
		for _, testData := range streamTestSuccessCollections["int64"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadInt64()).Equal(int64(0), errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
		...
		for _, testData := range streamTestSuccessCollections["uint64"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadUint64()).Equal(uint64(0), errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
		...
		for _, testData := range streamTestSuccessCollections["string"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadString()).Equal("", errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
...
func TestStream_ReadUnsafeString(t *testing.T) {
	...
	t.Run("read string utf8 error", func(t *testing.T) {
		...
		assert(stream.readUnsafeString()).Equal("", true, errors.ErrStream)
		assert(stream.GetReadPos()).Equal(streamPosBody)
		stream.Release()
	})
		...
		for _, testData := range streamTestSuccessCollections["bytes"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadBytes()).Equal(Bytes{}, errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
		...
		for _, testData := range streamTestSuccessCollections["array"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadArray()).Equal(Array{}, errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
		...
		for _, testData := range streamTestSuccessCollections["map"] {
			for _, i := range testRange {
				...
				for idx := i; idx < writePos-1; idx++ {
					stream.SetReadPos(i)
					stream.SetWritePos(idx)
					assert(stream.ReadMap()).Equal(Map{}, errors.ErrStream)
					assert(stream.GetReadPos()).Equal(i)
				}
				stream.Release()
			}
